			...
			<div class="page-wrap">
				<div id="page-inner">
					...
					<article class="container-body">
						<h3 class="h3-title">지난 수강응모 및 선정/대기자 리스트</h3>
						<div class="h3-btn"><a href="/mgr/entry/before/list" class="btn btn-level1 <c:if test="${empty mode or mode == 0 }">btn-default</c:if><c:if test="${mode == 1 }">btn-copy</c:if>" >수강응모 회원</a> 
						<a href="/mgr/entry/before/list?mode=1" class="btn btn-level1 <c:if test="${!empty mode and mode == 1 }">btn-default</c:if><c:if test="${empty mode or mode == 0 }">btn-copy</c:if>" >선정/대기자</a></div>
						<div class="h3-desc">항목별로 지난 수강신청 회원을 검색할 수 있습니다.</div>
						<h3 class="h3-title">지난 데이타 검색</h3>
						
						<form id="class_search" method="post" action="/mgr/entry/before/list">
						...
						<div class="by-2">
							<h3 class="tab-title">접수기간</h3>
							<div class="search-box">
								<span class="selectbox selectbox--type1">
								<select name="didx">
								<option value="0">접수기간 전체</option>
								<c:forEach items="${classDate }" var="item" varStatus="status">	
								<option <c:if test="${param.didx == item.idx_no }">selected</c:if> value="<c:out value="${item.idx_no}"/>"><c:out value="${fn:substring(item.startdate, 0, 10)}"/> ~ <c:out value="${fn:substring(item.enddate, 0, 10)}"/></option>
								</c:forEach>
								</select>
								</span>
							</div>
						...
						<!-- <div class="by-2">
							...
							<div class="rent-tabs ">
								<a href="#none" data-value="Y" class="select-class-result by-4 is-selected"><span>선정</span></a>
								<a href="#none" data-value="D" class="select-class-result by-4"><span>대기</span></a>
					...
					<!-- 	<div class="h3-desc">
						...
						<div class="btn-group">
							<a href="/mgr/entry/before/excel?mode=<c:out value="${mode }"/>&didx=<c:out value="${empty param.didx ? 0 : param.didx}"/>" class="btn btn-register btn-level2">엑셀로 다운로드 받기</a>
						</div>
